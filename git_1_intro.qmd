...
2) Aby skonfigurować Git, musisz zdefiniować kilka zmiennych globalnych: `user.name` i `user.email`. Obie są wymagane, abyś mógł dokonać zatwierdzenia. Ustaw swoją nazwę w `git bash` za pomocą następującego polecenia. Zastąp <USER_NAME>nazwą użytkownika, której chcesz użyć.
